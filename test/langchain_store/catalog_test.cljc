(ns langchain-store.catalog-test
  ...
  added without one is a citation nobody took. So this test refuses rows
  that skip the discipline, refuses :mirrors paths that don't exist (a
  citation pointing at a file this repo doesn't have is provenance for
  nothing), and refuses duplicate URLs (the same source twice is padding,
  not two citations).

  It cannot re-fetch the URLs — the suite runs with no network by design —
  so :http here is the recorded measurement, not a live one. What IS
  checked live is everything the repo itself can answer.

  Like the other portable test namespaces it runs on both runtimes and
  reads relative to the repo root, where `clojure -M:test` and the
  ClojureScript runner (`cljs.main --target node`) are both invoked."
...
(deftest catalog-parses-and-has-sources
  (let [c (catalog)]
    (is (map? c))
    (is (= :langchain-store/upstream-provenance (:catalog/id c)))
